...
var PIECE_NUM_X = 5;
var PIECE_NUM_Y = 5;
var PIECE_NUM = PIECE_NUM_X * PIECE_NUM_Y;
...
tm.define('GameScene', {
	...
	"init" : function() {
		...
		for (var i=0; i<PIECE_NUM_Y; i++) {
			for (var j=0; j<PIECE_NUM_X; j++) {
				var n = nums[ i * PIECE_NUM_X + j ];
				var piece = Piece(n).addChildTo(this.pieceGroup);
				...
				piece.onpointingstart = function() {
					if (this.number === self.currentNumber) {
						if (self.currentNumber == PIECE_NUM) { 
							...
							console.log('Game Clear: {0}'.format(time));
		...
		//=== タイトルボタン
		this.titleBtn = tm.app.FlatButton({
			'width' : 300 , 
			'height' : 100 , 
			'text' : "TITLE" , 
			'bgColor' : '#888'
		}).addChildTo(this);
		this.titleBtn.position.set(180,880);
		this.titleBtn.onpointingend = function() {
			self.app.replaceScene(TitleScene());
		};
		//=== リスタート
		this.restartBtn = tm.app.FlatButton({
			'width' : 300 , 
			'height' : 100 , 
			'text' : "RESTART" , 
			'bgColor' : '#888'
		}).addChildTo(this);
		this.restartBtn.position.set(500,880);
		this.restartBtn.onpointingend = function() {
	...
	"update" : function(app) {
		if (this.currentNumber <= PIECE_NUM) {
			var time = ((app.frame / app.fps) * 1000) | 0;
			var timeStr = "" + time;
			this.timerLabel.text = ((timeStr.length > 3) ? 
										timeStr.replace(/(\d)(?=(\d\d\d)+$)/g, "$1.") : 
										'0.' + timeStr);
		}
	}
});
...
tm.define('ResultScene' , {
	...
	"init" : function(param) {
		...
		this.tweetButton.onclick = function() {
			var tweetUrl = tm.social.Twitter.createURL({
				"type" : "tweet" , 
				"text" : "tmlib.js tutorial time: {time}".format(param) , 
